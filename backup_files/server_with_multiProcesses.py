...
TIMECONSUME_QUEUE_NAME = 'timeConsume_queue{}'.format(OFFLOAD)
MAX_RESULT_LEN = 128
################################################
...
def update_timeConsume_endUp(redi, time_name, time_queue_name):
    KEYNAME = time_name
    queue_time = int(redi.get(KEYNAME).decode('utf-8'))
    assert len(redi.lrange(time_queue_name, 0, -1)) > 0
    time_consume = int(redi.lindex(time_queue_name, 0).decode('utf-8'))
    redi.lpop(time_queue_name)

    new_timeConsume = queue_time - time_consume
    print('推理完当前input，此时队列估时为: {}'.format(new_timeConsume))
...
def update_timeConsume_startUp(redi, time_consume, time_name, time_queue_name):
    KEYNAME = time_name
    if redi.get(KEYNAME) == None:
        redi.set(KEYNAME, time_consume)
        redi.rpush(time_queue_name, time_consume)
        return time_consume
    else:
        redi.rpush(time_queue_name, time_consume)
        queue_time = int(redi.get(KEYNAME).decode('utf-8'))
...
def restart_webFront_clearRedis(redi):
    tmp_list = [QUEUE_NAME, TIMECONSUME_NAME, TIMECONSUME_QUEUE_NAME]
...
def input_data_constrain(in_dict, MAX_RESULT_LEN=MAX_RESULT_LEN):
...
def upload_then_loop_reading_obs_reply_file(uid, timeout, input_sentence):
    ...
    if finish_flag:
        ...
        update_timeConsume_endUp(r, TIMECONSUME_NAME, TIMECONSUME_QUEUE_NAME)
...
class S(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if self.path[:14] in paths:
            query = urlparse.urlparse(self.path)
            this_request_id = int(query[-2])
            reply_file = hf_work_file('reply', f'reply_{this_request_id}.json')
            print('请求uid is: {}'.format(this_request_id))
            try:
                with open(reply_file, 'r', encoding='utf-8') as fid:
                    raw_output = json.load(fid)['sentence']
                ...
                self.end_headers()
                self.wfile.write("{}".format('请求失败，请稍后再重试，谢谢！').encode('utf-8'))
        else:
            ...
            if self.path == '/inference_api':
                ...
                if not len(return_hint) == 2:
                    ...
                    if not uid == None:
                        ...
                        timeQueueConsume = update_timeConsume_startUp(r, time_consume, TIMECONSUME_NAME, TIMECONSUME_QUEUE_NAME)
                        print('排队推理此条输入，预计费时 {}s.'.format(timeQueueConsume))
                        ...
                        self.wfile.write(
                            '队列长度: {}, 预计排队时间: {}s, 当前请求uid为: {}'.format(queue_len, timeConsume,
                        ...
                        return None
                        # self.out = upload_then_loop_reading_obs_reply_file(uid, timeQueueConsume,
                        #                                                    input_sentence)
                        ...
                        self.send_response(300)
                        self.end_headers()
                        self.wfile.write("{}".format('当前请求队列已满，请稍后再试，谢谢！').encode('utf-8'))
                        return None
        else:
            self.send_response(404)
            self.end_headers()
